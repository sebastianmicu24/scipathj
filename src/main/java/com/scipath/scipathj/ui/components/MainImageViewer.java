 ...
 */
public class MainImageViewer extends JPanel implements ROIOverlay.ROIOverlayListener, ROIManager.ROIChangeListener {
    ...
    private JLabel createImageLabel() {
        ...
        label.setBackground(ThemeManager.isDarkTheme() ? new Color(40, 40, 40) : new Color(250, 250, 250));
        return label;
    }
    ...
    private void showEmptyState() {
        showStatePanel(FontAwesomeSolid.IMAGE, "No image selected",
    ...
    private void showLoadingState() {
        isLoading = true;
        showStatePanel(FontAwesomeSolid.SPINNER, "Loading image...", null,
                      UIManager.getColor("Label.disabledForeground"));
        imageInfoLabel.setText("Loading " + (currentImageFile != null ? currentImageFile.getName() : "image") + "...");
    }
    
    private void showErrorState(String errorMessage) {
        isLoading = false;
        showStatePanel(FontAwesomeSolid.EXCLAMATION_TRIANGLE, "Failed to load image",
                      errorMessage, UIConstants.ERROR_COLOR);
        imageInfoLabel.setText("Error loading " + (currentImageFile != null ? currentImageFile.getName() : "image"));
    }
    ...
    private void showStatePanel(FontAwesomeSolid icon, String message, String detail, Color color) {
        ...
        // Icon
        gbc.gridx = 0; gbc.gridy = 0; gbc.insets = new Insets(0, 0, UIConstants.LARGE_SPACING, 0);
        panel.add(UIUtils.createIconLabel(icon, icon == FontAwesomeSolid.IMAGE ? 64 : 48, color), gbc);
        
        // Main message
        gbc.gridy++; gbc.insets = new Insets(0, 0, detail != null ? UIConstants.MEDIUM_SPACING : 0, 0);
        panel.add(UIUtils.createBoldLabel(message, icon == FontAwesomeSolid.IMAGE ? UIConstants.SUBTITLE_FONT_SIZE : UIConstants.LARGE_FONT_SIZE), gbc);
        
        // Detail message
        if (detail != null) {
            gbc.gridy++; gbc.insets = new Insets(0, 0, 0, 0);
            JLabel detailLabel = UIUtils.createLabel("<html><center>" + detail + "</center></html>",
                                                   UIConstants.NORMAL_FONT_SIZE, UIManager.getColor("Label.disabledForeground"));
    ...
    private void updateImageInfo() {
        ...
        imageInfoLabel.setText(String.format("%s | %s | %s pixels | %s", fileName, fileSize, dimensions, extension));
    }
    ...
    private void setZoom(double newZoom, Point centerPoint) {
        ...
        if (scrollPosition != null) {
            ...
            SwingUtilities.invokeLater(() -> {
                JViewport viewport = scrollPane.getViewport();
                int x = Math.max(0, Math.min(finalScrollPosition.x,
                    layeredPane.getWidth() - viewport.getWidth()));
                int y = Math.max(0, Math.min(finalScrollPosition.y,
                    layeredPane.getHeight() - viewport.getHeight()));
    ...
    private void updateROIOverlayTransform() {
        ...
        LOGGER.debug("Updating ROI transform: actualScale=({:.6f},{:.6f}), viewPos=({},{}), imageSize=({}x{}), imagePos=({:.3f},{:.3f}), finalOffset=({:.3f},{:.3f})",
                    actualScaleX, actualScaleY, viewPosition.x, viewPosition.y,
                    (int)imageWidth, (int)imageHeight, imageX, imageY, offsetX, offsetY);
