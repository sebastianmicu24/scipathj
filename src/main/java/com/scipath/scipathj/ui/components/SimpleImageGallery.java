...
public class SimpleImageGallery extends JPanel {
    ...
    private void createStatusLabel() {
        ...
        statusLabel.setBorder(UIUtils.createPadding(UIConstants.SMALL_SPACING, 
    ...
    public void loadImagesFromFolder(File folder) {
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            LOGGER.error("Error loading images from folder: {}", folder.getAbsolutePath(), throwable);
    ...
    private void displayImages(File[] imageFiles) {
        ...
        for (File imageFile : imageFiles) {
            ...
            thumbnail.addPropertyChangeListener("thumbnailSelected", evt -> {
        ...
        LOGGER.info("Displayed {} images from folder: {}", imageFiles.length, currentFolder.getAbsolutePath());
    }
    ...
    private void showNoImagesFound() {
        thumbnailContainer.removeAll();
        thumbnailContainer.add(createStatePanel(FontAwesomeSolid.EXCLAMATION_TRIANGLE, 
            "No images found", UIConstants.WARNING_COLOR));
    ...
    private void showErrorState(String errorMessage) {
        thumbnailContainer.removeAll();
        thumbnailContainer.add(createStatePanel(FontAwesomeSolid.EXCLAMATION_CIRCLE, 
            "Error loading", UIConstants.ERROR_COLOR));
    ...
    private JPanel createStatePanel(FontAwesomeSolid icon, String message, Color iconColor) {
        ...
        panel.setBorder(UIUtils.createPadding(50, UIConstants.LARGE_SPACING, 50, UIConstants.LARGE_SPACING));
        
        // Icon
        JLabel iconLabel = new JLabel(FontIcon.of(icon, icon == FontAwesomeSolid.IMAGES ? 32 : 24));
        iconLabel.setForeground(iconColor != null ? iconColor : UIManager.getColor("Label.disabledForeground"));
        iconLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
        panel.add(iconLabel);
        
        panel.add(Box.createVerticalStrut(icon == FontAwesomeSolid.IMAGES ? 15 : UIConstants.MEDIUM_SPACING));
        
        // Message
        JLabel messageLabel = new JLabel(message);
        messageLabel.setFont(messageLabel.getFont().deriveFont(
            icon == FontAwesomeSolid.IMAGES ? Font.BOLD : Font.PLAIN, 
            icon == FontAwesomeSolid.IMAGES ? UIConstants.NORMAL_FONT_SIZE : UIConstants.SMALL_FONT_SIZE));
        messageLabel.setForeground(iconColor != null ? iconColor : UIManager.getColor("Label.disabledForeground"));
    ...
    public List<File> getAllImageFiles() {
        return thumbnails.stream()
                .map(SimpleImageThumbnail::getImageFile)
                .toList();
    }
