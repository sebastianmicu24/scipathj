...
public class ROIManager {
    ...
    public boolean removeROI(String roiId) {
        for (List<UserROI> rois : imageROIs.values()) {
            ...
            if (toRemove != null) {
                ...
                LOGGER.info("Removed ROI '{}' from image '{}'", toRemove.getName(), toRemove.getImageFileName());
    ...
    public void saveAllROIsToMasterZip(File outputFile) throws IOException {
        ...
        File tempDir = new File(System.getProperty("java.io.tmpdir"), "scipathj_master_rois_" + System.currentTimeMillis());
        tempDir.mkdirs();
    ...
    private void saveSingleROIAsZip(UserROI userROI, File outputFile) throws IOException {
        ...
        File tempDir = new File(System.getProperty("java.io.tmpdir"), "scipathj_single_roi_" + System.currentTimeMillis());
        tempDir.mkdirs();
    ...
    private void saveMultipleROIs(List<UserROI> rois, File outputFile) throws IOException {
        ...
        File tempDir = new File(System.getProperty("java.io.tmpdir"), "scipathj_rois_" + System.currentTimeMillis());
        tempDir.mkdirs();
        ...
        if (userROI.hasComplexShape()) {
            ...
            switch (userROI.getType()) {
                ...
                case CIRCLE:
                    ...
                    roi = new ij.gui.OvalRoi(userROI.getX(), userROI.getY(), userROI.getWidth(), userROI.getHeight());
                    break;
                default:
                    roi = new Roi(userROI.getX(), userROI.getY(), userROI.getWidth(), userROI.getHeight());
                    break;
