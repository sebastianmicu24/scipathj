...
public class SimpleImageThumbnail extends JPanel {
  ...
  private void createImageLabel() {
        ...
        new JLabel() {
          ...
          protected void paintComponent(Graphics g) {
            if (getIcon() != null) {
              ...
              g2d.setClip(
                  UIUtils.createRoundedRectangle(
                      0, 0, getWidth(), getHeight(), UIConstants.BORDER_RADIUS));
    ...
    imageLabel.setPreferredSize(
        new Dimension(UIConstants.THUMBNAIL_SIZE, UIConstants.THUMBNAIL_SIZE));
  ...
  private void loadThumbnailAsync() {
    CompletableFuture.supplyAsync(
        ...
        .exceptionally(
            throwable -> {
              LOGGER.error(
                  "Error in thumbnail loading task for: {}", imageFile.getName(), throwable);
  ...
  private void setThumbnailImage(BufferedImage thumbnail) {
    ...
    Image scaledImage =
        thumbnail.getScaledInstance(
            UIConstants.THUMBNAIL_SIZE, UIConstants.THUMBNAIL_SIZE, Image.SCALE_SMOOTH);
