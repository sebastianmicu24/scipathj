...
public class AnalysisPipeline {
    ...
    public AnalysisPipeline() {
        this.vesselSettings = ConfigurationManager.getInstance().initializeVesselSegmentationSettings();
        this.nuclearSettings = ConfigurationManager.getInstance().initializeNuclearSegmentationSettings();
        this.cytoplasmSettings = ConfigurationManager.getInstance().initializeCytoplasmSegmentationSettings();
     ...
     */
    public AnalysisPipeline(VesselSegmentationSettings vesselSettings,
                           NuclearSegmentationSettings nuclearSettings) {
        this.vesselSettings = vesselSettings != null ? vesselSettings :
            ConfigurationManager.getInstance().initializeVesselSegmentationSettings();
        this.nuclearSettings = nuclearSettings != null ? nuclearSettings :
            ConfigurationManager.getInstance().initializeNuclearSegmentationSettings();
        this.cytoplasmSettings = ConfigurationManager.getInstance().initializeCytoplasmSegmentationSettings();
        this.mainSettings = MainSettings.getInstance();
        this.roiManager = ROIManager.getInstance();
    ...
    public AnalysisPipeline(VesselSegmentationSettings vesselSettings,
        ...
        this.nuclearSettings = nuclearSettings != null ? nuclearSettings :
            ConfigurationManager.getInstance().initializeNuclearSegmentationSettings();
        this.cytoplasmSettings = cytoplasmSettings != null ? cytoplasmSettings :
            ConfigurationManager.getInstance().initializeCytoplasmSegmentationSettings();
        this.mainSettings = MainSettings.getInstance();
        this.roiManager = ROIManager.getInstance();
    ...
    public AnalysisResults processBatch(File[] imageFiles) {
        ...
        try {
            for (int i = 0; i < imageFiles.length; i++) {
                ...
                updateProgress(i, "Processing image " + (i + 1) + "/" + imageFiles.length + ": " + fileName);
                ...
                try {
                    ...
                    if (result.isSuccess()) {
                        ...
                        LOGGER.info("Completed analysis for {} - Vessels: {}, Nuclei: {}, Cells: {}",
                                   fileName, result.getVesselCount(), result.getNucleusCount(), result.getCellCount());
                    } else {
            ...
            updateProgress(100, String.format("Batch analysis completed! Found %d vessels, %d nuclei, and %d cells across %d images.",
                                             totalVessels, totalNuclei, totalCells, successfulImages));
            
            LOGGER.info("Batch analysis completed: {} images processed, {} vessels, {} nuclei, {} cells",
    ...
    public ImageAnalysisResult processImage(ImagePlus imagePlus, String fileName) {
        try {
            ...
            VesselSegmentation vesselSegmentation = new VesselSegmentation(imagePlus, fileName, vesselSettings);
            ...
            SimpleHENuclearSegmentation nuclearSegmentation = new SimpleHENuclearSegmentation(imagePlus, fileName, nuclearSettings);
            ...
            try {
                ...
                if (isAvailable) {
                    ...
                    LOGGER.info("Step 2 complete: Found {} nuclei in image: {}", nucleusROIs.size(), fileName);
                } else {
            ...
            if (!nucleusROIs.isEmpty()) {
                ...
                List<UserROI> vesselROIsForExclusion = cytoplasmSettings.isExcludeVessels() ?
                    vesselROIs : List.of();
                
                CytoplasmSegmentation cytoplasmSegmentation = new CytoplasmSegmentation(
                    imagePlus, fileName, vesselROIsForExclusion, nucleusROIs, cytoplasmSettings);
                
                cytoplasmROIs = cytoplasmSegmentation.segmentCytoplasm();
                cellROIs = cytoplasmSegmentation.getCellROIs();
                
                LOGGER.debug("Step 3 complete: Found {} cells and {} cytoplasm regions in image: {}",
                           cellROIs.size(), cytoplasmROIs.size(), fileName);
            } else {
            ...
            return ImageAnalysisResult.success(fileName, vesselROIs.size(), nucleusROIs.size(), cellROIs.size());
    ...
    public static class AnalysisResults {
        ...
        public int getTotalNuclei() { return totalNuclei; }
        public int getTotalCells() { return totalCells; }
        
        @Override
        public String toString() {
            return String.format("AnalysisResults[images=%d, vessels=%d, nuclei=%d, cells=%d]",
    ...
    public static class ImageAnalysisResult {
        ...
        private ImageAnalysisResult(String fileName, boolean success, String errorMessage,
                                   int vesselCount, int nucleusCount, int cellCount) {
        ...
        public static ImageAnalysisResult success(String fileName, int vesselCount, int nucleusCount, int cellCount) {
        ...
        public int getNucleusCount() { return nucleusCount; }
        public int getCellCount() { return cellCount; }
        
        @Override
        public String toString() {
            return success ?
                String.format("ImageAnalysisResult[%s: vessels=%d, nuclei=%d, cells=%d]", fileName, vesselCount, nucleusCount, cellCount) :
