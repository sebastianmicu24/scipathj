...
public class SimpleHENuclearSegmentation {
  ...
  private static final java.util.Set<String> EXECUTING_IMAGES =
      java.util.concurrent.ConcurrentHashMap.newKeySet();
  private static final java.util.concurrent.atomic.AtomicInteger EXECUTION_COUNTER =
   ...
   */
  public SimpleHENuclearSegmentation(
    ...
    this.settings = configurationManager.initializeNuclearSegmentationSettings();

    DirectFileLogger.logStarDist(
        "INFO", "=== SimpleHENuclearSegmentation Constructor [ID:" + executionId + "] ===");
    DirectFileLogger.logStarDist(
        "INFO", "Image: " + imageFileName + ", Execution ID: " + executionId);
    DirectFileLogger.logStarDist("INFO", "Total executions so far: " + EXECUTION_COUNTER.get());

    initializeContext();

    LOGGER.debug(
        "SimpleHENuclearSegmentation initialized with default settings for image: {} [ID:{}]",
        imageFileName,
        executionId);
  }
  ...
  public SimpleHENuclearSegmentation(
    ...
    this.roiManager = ROIManager.getInstance();
    this.settings =
        settings != null ? settings : configurationManager.initializeNuclearSegmentationSettings();
    ...
    LOGGER.debug(
        "SimpleHENuclearSegmentation initialized with custom settings for image: {} [ID:{}]",
        imageFileName,
        executionId);
  }
  ...
  private void initializeContext() {
    try {
      ...
      DirectFileLogger.logStarDist(
          "INFO", "=== Initializing SciJava context for H&E nuclear segmentation ===");

      // Set up TensorFlow model cache directory to avoid permission issues
      setupTensorFlowCache();

      // Create a SciJava context with required services (excluding ScriptService for JPackage
      // compatibility)
      LOGGER.info("Creating SciJava context with required services...");
      DirectFileLogger.logStarDist(
          "INFO",
          "Creating SciJava context (excluding ScriptService for JPackage compatibility)...");
      ...
      if (isJPackage) {
        ...
        DirectFileLogger.logStarDist(
            "INFO", "Using JPackage-optimized SciJava context with all required services");
        this.context =
            new Context(
      ...
      DirectFileLogger.logStarDist(
          "INFO",
          "✓ SciJava context initialized successfully with "
              + context.getServiceIndex().size()
              + " services");

      // Log TensorFlow service availability
      try {
        net.imagej.tensorflow.TensorFlowService tfService =
            context.getService(net.imagej.tensorflow.TensorFlowService.class);
        LOGGER.info("TensorFlow service available: {}", tfService != null);
        DirectFileLogger.logStarDist(
            "INFO", "TensorFlow service available: " + (tfService != null));
        if (tfService != null) {
          LOGGER.info("TensorFlow service class: {}", tfService.getClass().getName());
          DirectFileLogger.logStarDist(
              "INFO", "TensorFlow service class: " + tfService.getClass().getName());
        }
      } catch (Exception e) {
  ...
  private void setupTensorFlowCache() {
    try {
      ...
      DirectFileLogger.logStarDist(
          "INFO", "Setting up TensorFlow cache (JPackage: " + isJPackage + ")");
      ...
      DirectFileLogger.logStarDist(
          "INFO", "TensorFlow cache directory: " + cacheDir.getAbsolutePath());
      ...
      if (!cacheDir.exists()) {
        ...
        if (created) {
          ...
          LOGGER.error(
              "Failed to create TensorFlow cache directory: {}", cacheDir.getAbsolutePath());
  ...
  private void setupJPackageTensorFlowEnvironment(String appImage, java.io.File cacheDir) {
    try {
      DirectFileLogger.logStarDist(
          "INFO", "=== Setting up JPackage-specific TensorFlow environment ===");

      // Detect GUI vs Console execution context
      boolean isGuiContext = detectGuiExecutionContext();
      DirectFileLogger.logStarDist(
          "INFO",
          "Execution context: " + (isGuiContext ? "GUI (double-click)" : "Console (terminal)"));

      // CRITICAL FIX: Set working directory to JPackage app directory to resolve resource
      // dependencies
      ...
      DirectFileLogger.logStarDist(
          "INFO", "✓ Set working directory to JPackage app directory: " + appDir.getAbsolutePath());
      ...
      DirectFileLogger.logStarDist(
          "INFO", "Set TensorFlow working directory: " + tensorflowWorkingDir.getAbsolutePath());
      ...
      if (libDir.exists()) {
        ...
        DirectFileLogger.logStarDist(
            "INFO", "Added JPackage lib directory: " + libDir.getAbsolutePath());
      }
      ...
      if (libsDir.exists()) {
        ...
        DirectFileLogger.logStarDist(
            "INFO", "Added JPackage libs directory: " + libsDir.getAbsolutePath());
      }
      ...
      System.setProperty(
          "tensorflow.native.path",
          libDir.exists() ? libDir.getAbsolutePath() : appDir.getAbsolutePath());
      ...
      DirectFileLogger.logStarDist(
          "INFO", "Set TensorFlow model extract directory: " + modelExtractDir.getAbsolutePath());
      ...
      try {
        ...
        DirectFileLogger.logStarDist(
            "WARN", "Could not force reload of native library path: " + e.getMessage());
      }

      DirectFileLogger.logStarDist(
          "INFO", "✓ JPackage-specific TensorFlow environment setup complete");
  ...
  private boolean detectGuiExecutionContext() {
    try {
      ...
      try {
        ...
        if (osName.contains("windows")) {
          ...
          if (parent != null) {
            ...
            isGuiParent =
                parentCommand.contains("explorer")
      ...
      DirectFileLogger.logStarDist(
          "WARN", "Failed to detect execution context, assuming GUI: " + e.getMessage());
  ...
  private void initializeTensorFlowForGui(java.io.File libDir) {
    try {
      ...
      try {
        ...
        DirectFileLogger.logStarDist(
            "INFO", "✓ TensorFlow initialized in GUI context, version: " + version);

      } catch (Exception e) {
        DirectFileLogger.logStarDist(
            "WARN", "TensorFlow GUI initialization failed: " + e.getMessage());
      ...
      DirectFileLogger.logStarDist(
          "WARN", "TensorFlow GUI initialization encountered issues: " + e.getMessage());
    }
  }
  ...
  public List<NucleusROI> segmentNuclei() throws Exception {
    ...
    if (EXECUTING_IMAGES.contains(imageFileName)) {
      DirectFileLogger.logStarDist(
          "WARN", "=== DUPLICATE EXECUTION DETECTED [ID:" + executionId + "] ===");
      DirectFileLogger.logStarDist(
          "WARN", "Image '" + imageFileName + "' is already being processed!");
    ...
    try {
      LOGGER.info(
          "Starting H&E nuclear segmentation for image '{}' [ID:{}]", imageFileName, executionId);
      DirectFileLogger.logStarDist(
          "INFO", "=== Starting H&E nuclear segmentation [ID:" + executionId + "] ===");
      DirectFileLogger.logStarDist(
          "INFO", "Image: " + imageFileName + ", Execution ID: " + executionId);
      ...
      DirectFileLogger.logStarDist(
          ...
          "Image details: "
              ...
              + ", "
              + originalImage.getNChannels()
              + " channels, "
              + originalImage.getBitDepth()
              + "-bit");
      ...
      DirectFileLogger.logStarDist(
          "INFO",
          "StarDist execution completed, found "
              + nucleiROIs.size()
              + " nuclei [ID:"
              + executionId
              + "]");
      ...
      LOGGER.info(
          "H&E nuclear segmentation completed. Found {} nuclei [ID:{}]",
          nucleiROIs.size(),
          executionId);
      DirectFileLogger.logStarDist(
          "INFO",
          "=== H&E nuclear segmentation completed successfully [ID:" + executionId + "] ===");

      return nucleiROIs;

    } catch (Exception e) {
      DirectFileLogger.logStarDistException(
          "StarDist nuclear segmentation failed for image: "
              + imageFileName
              + " [ID:"
              + executionId
              + "]",
      ...
      DirectFileLogger.logStarDist(
          "INFO", "Removed '" + imageFileName + "' from executing set [ID:" + executionId + "]");
    }
  }
  ...
  private void checkStarDistAvailability() throws Exception {
    try {
      ...
      DirectFileLogger.logStarDist(
          "INFO", "✓ CSBDeep GenericNetwork class found: " + csbDeepClass.getName());
      ...
      try {
        ...
        DirectFileLogger.logStarDist(
            "WARN", "Continuing with StarDist execution despite TensorFlow validation failure");
      }
  ...
  private void validateTensorFlowNativeLibraries() throws Exception {
    try {
      ...
      if (isJPackage) {
        ...
        if (libDir.exists()) {
          ...
          if (libFiles != null && libFiles.length > 0) {
            DirectFileLogger.logStarDist(
                "INFO", "Found " + libFiles.length + " potential TensorFlow native libraries:");
            for (java.io.File libFile : libFiles) {
              DirectFileLogger.logStarDist(
                  "INFO", "  - " + libFile.getName() + " (" + libFile.length() + " bytes)");
            }
          } else {
            DirectFileLogger.logStarDist(
                "WARN", "No TensorFlow native libraries found in lib directory");
          }
        }
      }
      ...
      try {
        ...
        if (isJPackage) {
          DirectFileLogger.logStarDist(
              "ERROR", "This is a JPackage environment. The issue is likely:");
          DirectFileLogger.logStarDist(
              "ERROR", "1. TensorFlow native libraries are not included in the package");
          DirectFileLogger.logStarDist("ERROR", "2. Native library path is not set correctly");
          DirectFileLogger.logStarDist(
              "ERROR", "3. Native libraries are not compatible with the target system");
          ...
          if (osName.contains("windows")) {
            DirectFileLogger.logStarDist(
                "ERROR", "For Windows, ensure tensorflow_jni.dll is in the lib directory");
          } else if (osName.contains("linux")) {
            DirectFileLogger.logStarDist(
                "ERROR", "For Linux, ensure libtensorflow_jni.so is in the lib directory");
          } else if (osName.contains("mac")) {
            DirectFileLogger.logStarDist(
                "ERROR", "For macOS, ensure libtensorflow_jni.dylib is in the lib directory");
          }
        }
  ...
  private Dataset convertToDataset(ImagePlus imagePlus) {
    try {
      ...
      LOGGER.debug(
          "Converted ImagePlus to Dataset: {}x{} pixels", dataset.getWidth(), dataset.getHeight());
  ...
  private ImagePlus prepareImageForStarDist(ImagePlus imagePlus) {
    LOGGER.debug(
        "Preparing image for StarDist H&E model: {}x{}, {} channels, {}-bit, type={}",
        imagePlus.getWidth(),
        imagePlus.getHeight(),
    ...
    if (imagePlus.getBitDepth() != 8) {
      ...
      LOGGER.debug(
          "Converted to 8-bit: {}x{}, {} channels, {}-bit, type={}",
          converted.getWidth(),
          converted.getHeight(),
          converted.getNChannels(),
          converted.getBitDepth(),
          converted.getType());
  ...
  private ImagePlus convertRGBToChannelStack(ImagePlus rgbImage) {
    try {
      ...
      LOGGER.debug(
          "Created RGB channel stack: {}x{}, {} channels, {}-bit, type={}",
          result.getWidth(),
          result.getHeight(),
          result.getNChannels(),
          result.getBitDepth(),
      ...
      LOGGER.debug(
          "Fallback grayscale: {}x{}, {} channels, {}-bit, type={}",
          gray.getWidth(),
          gray.getHeight(),
          gray.getNChannels(),
          gray.getBitDepth(),
  ...
  private List<NucleusROI> executeStarDistHE(Dataset inputDataset) throws Exception {
    ...
    DirectFileLogger.logStarDist(
        "INFO", "Executing StarDist2D with H&E model choice: " + settings.getModelChoice());

    LOGGER.info(
        "Dataset dimensions: {}x{}, channels: {}",
        inputDataset.getWidth(),
        inputDataset.getHeight(),
        inputDataset.getChannels());
    DirectFileLogger.logStarDist(
        "INFO",
        "Dataset dimensions: "
            + inputDataset.getWidth()
    ...
    if (ijRoiManager != null) {
      ...
      DirectFileLogger.logStarDist(
          "INFO", "Clearing existing ROI Manager with " + ijRoiManager.getCount() + " ROIs");
      ijRoiManager.reset();
    }
    ...
    try {
      ...
      LOGGER.info(
          "Model choice: {}, Prob threshold: {}, NMS threshold: {}",
          params.get("modelChoice"),
          params.get("probThresh"),
          params.get("nmsThresh"));
      DirectFileLogger.logStarDist(
          "INFO",
          "Model: "
              + params.get("modelChoice")
      ...
      try {
        ...
        if (result == null) {
          ...
          throw new Exception(
              "StarDist2D command returned null result - this usually indicates TensorFlow model"
                  + " execution failure in JPackage environment");
        }
        ...
        if (result.getOutputs() != null && !result.getOutputs().isEmpty()) {
          ...
          DirectFileLogger.logStarDist(
              "INFO", "StarDist2D command outputs: " + result.getOutputs().keySet());
          ...
          DirectFileLogger.logStarDist(
              "WARN",
              "✗ StarDist2D command produced no outputs - this indicates CSBDeep GenericNetwork"
                  + " failed");
        }
        ...
        DirectFileLogger.logStarDist(
            "INFO", "StarDist detected " + detectedRois.length + " ROIs in ROI Manager");

        if (detectedRois.length == 0) {
          LOGGER.warn(
              "StarDist detected 0 ROIs - this might indicate a problem with the model or input");
          DirectFileLogger.logStarDist(
              "WARN",
              "StarDist detected 0 ROIs - this might indicate a problem with the model or input");
        }
        ...
        DirectFileLogger.logStarDist(
            "INFO",
            "✓ Successfully converted " + nucleusROIs.size() + " ROIs to NucleusROI objects");

        return nucleusROIs;

      } catch (java.util.concurrent.ExecutionException ee) {
        DirectFileLogger.logStarDist(
            "ERROR", "✗ StarDist2D command execution failed with ExecutionException");
        ...
        if (cause != null) {
          DirectFileLogger.logStarDist(
              "ERROR",
              "Underlying cause: " + cause.getClass().getSimpleName() + ": " + cause.getMessage());
          ...
          if (cause.getMessage() != null) {
            ...
            if (msg.contains("tensorflow") || msg.contains("native") || msg.contains("jni")) {
              DirectFileLogger.logStarDist(
                  "ERROR", "This appears to be a TensorFlow native library loading issue");
              DirectFileLogger.logStarDist(
                  "ERROR",
                  "In JPackage environment, ensure TensorFlow JNI libraries are properly included");
            }
          }
        }
  ...
  private void logRuntimeEnvironment() {
    try {
      ...
      try {
        ...
        DirectFileLogger.logStarDist(
            "INFO",
            "StarDist2DModel class loaded from: "
                + starDistModelClass.getProtectionDomain().getCodeSource().getLocation());
      } catch (Exception e) {
        DirectFileLogger.logStarDist(
            "ERROR", "Failed to get StarDist2DModel location: " + e.getMessage());
      }
  ...
  private List<NucleusROI> performSimpleNucleiDetection(ImagePlus image) {
    ...
    try {
      LOGGER.debug(
          "Performing simple nuclei detection on {}x{} image", image.getWidth(), image.getHeight());
      ...
      ij.plugin.filter.ParticleAnalyzer pa =
          new ij.plugin.filter.ParticleAnalyzer(
              ij.plugin.filter.ParticleAnalyzer.ADD_TO_MANAGER
                  | ij.plugin.filter.ParticleAnalyzer.EXCLUDE_EDGE_PARTICLES,
  ...
  private Map<String, Object> createHEParameters(Dataset inputDataset) {
    ...
    LOGGER.debug(
        "Created H&E parameters: model={}, probThresh={}, nmsThresh={}, normalize={},"
            + " percentiles={}-{}",
        settings.getModelChoice(),
        settings.getProbThresh(),
        settings.getNmsThresh(),
        settings.isNormalizeInput(),
  ...
  private List<NucleusROI> convertToNucleusROIs(Roi[] rois) {
    ...
    for (int i = 0; i < rois.length; i++) {
      ...
      LOGGER.debug(
          "Created nucleus ROI: {} with area: {:.1f} pixels",
          nucleusName,
          nucleusROI.getNucleusArea());
    }
  ...
  public String toString() {
    return String.format(
        "SimpleHENuclearSegmentation[image=%s, model=%s, probThresh=%.3f, available=%s]",
