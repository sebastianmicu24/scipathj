...
public class CytoplasmSegmentation {
     ...
     */
    public CytoplasmSegmentation(ImagePlus originalImage, String imageFileName,
                                List<UserROI> vesselROIs, List<NucleusROI> nucleusROIs) {
        ...
        this.settings = ConfigurationManager.getInstance().initializeCytoplasmSegmentationSettings();
        ...
        LOGGER.debug("CytoplasmSegmentation initialized for image: {} with {} nuclei and {} vessels",
                    imageFileName, this.nucleusROIs.size(), this.vesselROIs.size());
    }
    ...
    public CytoplasmSegmentation(ImagePlus originalImage, String imageFileName,
        ...
        this.vesselROIs = vesselROIs != null ? vesselROIs : new ArrayList<>();
        this.nucleusROIs = nucleusROIs != null ? nucleusROIs : new ArrayList<>();
        this.settings = settings != null ? settings : ConfigurationManager.getInstance().initializeCytoplasmSegmentationSettings();
        this.mainSettings = MainSettings.getInstance();
        this.roiManager = ROIManager.getInstance();
        this.cellROIs = new ArrayList<>();
        this.cytoplasmROIs = new ArrayList<>();
        ...
        LOGGER.debug("CytoplasmSegmentation initialized for image: {} with custom settings", imageFileName);
    }
    ...
    public List<CytoplasmROI> segmentCytoplasm() {
        LOGGER.info("Starting cytoplasm segmentation using Voronoi tessellation for image: {}", imageFileName);
        ...
        try {
            ...
            LOGGER.info("Cytoplasm segmentation completed. Created {} cells and {} cytoplasm ROIs",
                       cellROIs.size(), cytoplasmROIs.size());
     ...
     */
    private void processNucleiForCellCreation(ImagePlus cytoplasmImage, int imageWidth, int imageHeight) {
    ...
    private CytoplasmROI createCytoplasmROI(Roi cellRoi, NucleusROI nucleusROI, int nucleusNumber) {
        try {
            ...
            if (cytoplasmShape != null) {
                ...
                CytoplasmROI cytoplasm = new CytoplasmROI(cytoplasmShape, imageFileName, cytoplasmName, nucleusROI);
            ...
            LOGGER.warn("Failed to create cytoplasm ROI for nucleus {}: {}", nucleusNumber, e.getMessage());
        }
    ...
    private boolean isValidCell(Roi cellRoi) {
        ...
        if (settings.isValidateCellShape()) {
            ...
            double aspectRatio = (double) Math.max(bounds.width, bounds.height) / Math.min(bounds.width, bounds.height);
    ...
    public String getStatistics(List<CytoplasmROI> cytoplasmROIs) {
        ...
        double totalArea = cytoplasmROIs.stream()
            .mapToDouble(CytoplasmROI::getCytoplasmArea)
            .sum();
        
        double avgArea = totalArea / cytoplasmROIs.size();
        
        double minArea = cytoplasmROIs.stream()
            .mapToDouble(CytoplasmROI::getCytoplasmArea)
            .min()
            .orElse(0.0);
        
        double maxArea = cytoplasmROIs.stream()
            .mapToDouble(CytoplasmROI::getCytoplasmArea)
            .max()
            .orElse(0.0);
        
        long linkedCount = cytoplasmROIs.stream()
            .mapToLong(c -> c.hasAssociatedNucleus() ? 1 : 0)
            .sum();
        
        return String.format(
            "Cytoplasm: %d regions, Total area: %.1f px, Avg area: %.1f px (range: %.1f-%.1f), Linked: %d",
