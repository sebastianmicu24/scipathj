    ...
    public static synchronized void applyFix() {
        ...
        try {
            ...
            if (isJava9Plus() && !isURLClassLoader(originalClassLoader)) {
                LOGGER.debug("Detected Java 9+ with non-URLClassLoader: {}", originalClassLoader.getClass().getName());
                ...
                LOGGER.debug("ClassLoader fix not needed - already compatible: {}", originalClassLoader.getClass().getName());
            }
