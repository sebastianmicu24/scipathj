...
public class ClassLoaderDebugger {
    ...
    public static void identifyPotentialIssues() {
        ...
        if (!(systemCL instanceof URLClassLoader)) {
            LOGGER.warn("ISSUE: System ClassLoader is not URLClassLoader: {}", systemCL.getClass().getName());
        ...
        if (!(contextCL instanceof URLClassLoader)) {
            LOGGER.warn("ISSUE: Context ClassLoader is not URLClassLoader: {}", contextCL.getClass().getName());
        }
