...
public class NuclearSegmentationSettings {
    ...
    public boolean isValid() {
        ...
        if (percentileBottom >= percentileTop) {
            LOGGER.warn("Invalid percentile range: bottom ({}) >= top ({})", percentileBottom, percentileTop);
            valid = false;
        }
        
        if (minNucleusSize >= maxNucleusSize) {
            LOGGER.warn("Invalid nucleus size range: min ({}) >= max ({})", minNucleusSize, maxNucleusSize);
            valid = false;
        }
    ...
    public String toString() {
        return String.format(
            ...
            modelChoice, probThresh, nmsThresh, percentileBottom, percentileTop,
            minNucleusSize, maxNucleusSize, nTiles
        );
    }
}