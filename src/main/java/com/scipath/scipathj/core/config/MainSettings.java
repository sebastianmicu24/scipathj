...
public class MainSettings {
    ...
    public enum ROICategory {
        ...
        ROICategory(String displayName, Color defaultBorderColor, float defaultFillOpacity, int defaultBorderWidth) {
        ...
        public Color getDefaultBorderColor() { return defaultBorderColor; }
        public float getDefaultFillOpacity() { return defaultFillOpacity; }
        public int getDefaultBorderWidth() { return defaultBorderWidth; }
    }
    ...
    public static class ROIAppearanceSettings {
        ...
        private void updateFillColor() {
            ...
            this.fillColor = new Color(borderColor.getRed(), borderColor.getGreen(),
    ...
    public static final double DEFAULT_PIXELS_PER_MICROMETER = 1.0; // 1 pixel = 1 micrometer by default
    ...
    this.cytoplasmSettings = new ROIAppearanceSettings(
    ...
    public void setRoiBorderColor(Color roiBorderColor) {
        ...
        LOGGER.debug("Vessel ROI border color set to: RGB({}, {}, {})",
                    roiBorderColor.getRed(), roiBorderColor.getGreen(), roiBorderColor.getBlue());
    }
     ...
     */
    public void updateCategorySettings(ROICategory category, Color borderColor, float fillOpacity, int borderWidth) {
        ...
        notifySettingsChanged();
        LOGGER.debug("Updated {} settings: color=RGB({},{},{}), opacity={}, width={}",
                    category.getDisplayName(), borderColor.getRed(), borderColor.getGreen(),
                    borderColor.getBlue(), fillOpacity, borderWidth);
    }
    ...
    private String formatROISettings(String type, ROIAppearanceSettings settings) {
        return String.format("%s[RGB(%d,%d,%d),opacity=%.2f,width=%d]",
                           type,
    ...
    private void validateROISettings(String categoryName, ROIAppearanceSettings settings) {
        ...
        if (settings.getFillOpacity() < 0.0f || settings.getFillOpacity() > 1.0f) {
            throw new IllegalStateException("Invalid " + categoryName + " fill opacity: " + settings.getFillOpacity());
        }
        if (settings.getBorderWidth() < 1) {
            throw new IllegalStateException("Invalid " + categoryName + " border width: " + settings.getBorderWidth());
        }
    }
}