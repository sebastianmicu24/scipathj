...
public class StarDist2DNMS extends StarDist2DBase implements Command {
  ...
  @Parameter(
      ...
      choices = {Opt.ROI_POSITION_STACK, Opt.ROI_POSITION_HYPERSTACK},
      style = ChoiceWidget.RADIO_BUTTON_HORIZONTAL_STYLE)
  ...
  public void run() {
    ...
    final RandomAccessibleInterval<FloatType> probRAI =
        (RandomAccessibleInterval<FloatType>) prob.getImgPlus();
    final RandomAccessibleInterval<FloatType> distRAI =
        (RandomAccessibleInterval<FloatType>) dist.getImgPlus();
    ...
    final AxisType[] axesOut =
        isTimelapse ? new AxisType[] {Axes.X, Axes.Y, Axes.TIME} : new AxisType[] {Axes.X, Axes.Y};

    if (isTimelapse) {
      final int probTimeDim =
          IntStream.range(0, probAxes.size())
              .filter(d -> prob.axis(d).type() == Axes.TIME)
              .findFirst()
              .getAsInt();
      final int distTimeDim =
          IntStream.range(0, distAxes.size())
              .filter(d -> dist.axis(d).type() == Axes.TIME)
              .findFirst()
              .getAsInt();
      ...
      for (int t = 0; t < numFrames; t++) {
        final Candidates polygons =
            new Candidates(
                Views.hyperSlice(probRAI, probTimeDim, t),
                Views.hyperSlice(distRAI, distTimeDim, t),
                probThresh,
                excludeBoundary,
                verbose ? log : null);
        ...
        if (verbose)
          log.info(
              String.format(
                  "frame %03d: %d polygon candidates, %d remain after non-maximum suppression",
                  t, polygons.getSorted().size(), polygons.getWinner().size()));
        export(outputType, polygons, 1 + t, numFrames, roiPosition);
      }
    } else {
      final Candidates polygons =
          new Candidates(probRAI, distRAI, probThresh, excludeBoundary, verbose ? log : null);
      polygons.nms(nmsThresh);
      if (verbose)
        log.info(
            String.format(
                "%d polygon candidates, %d remain after non-maximum suppression",
  ...
  private boolean checkInputs() {
    ...
    if (!((prob.numDimensions() == 2 && probAxes.containsAll(Arrays.asList(Axes.X, Axes.Y)))
        || (prob.numDimensions() == 3
            && probAxes.containsAll(Arrays.asList(Axes.X, Axes.Y, Axes.TIME)))))
      return showError(String.format("%s must be a 2D image or timelapse.", Opt.PROB_IMAGE));

    if (!((dist.numDimensions() == 3
            && distAxes.containsAll(Arrays.asList(Axes.X, Axes.Y, Axes.CHANNEL))
            && dist.getChannels() >= 3)
        || (dist.numDimensions() == 4
            && distAxes.containsAll(Arrays.asList(Axes.X, Axes.Y, Axes.CHANNEL, Axes.TIME))
            && dist.getChannels() >= 3)))
      return showError(
          String.format(
              "%s must be a 2D image or timelapse with at least three channels.", Opt.DIST_IMAGE));

    if ((prob.numDimensions() + 1) != dist.numDimensions())
      return showError(
          String.format("Axes of %s and %s not compatible.", Opt.PROB_IMAGE, Opt.DIST_IMAGE));

    if (prob.getWidth() != dist.getWidth() || prob.getHeight() != dist.getHeight())
      return showError(
          String.format("Width or height of %s and %s differ.", Opt.PROB_IMAGE, Opt.DIST_IMAGE));

    if (prob.getFrames() != dist.getFrames())
      return showError(
          String.format("Number of frames of %s and %s differ.", Opt.PROB_IMAGE, Opt.DIST_IMAGE));
    ...
    if (excludeBoundary < 0) return showError(String.format("%s must be >= 0", Opt.EXCLUDE_BNDRY));

    if (!(outputType.equals(Opt.OUTPUT_ROI_MANAGER)
        || outputType.equals(Opt.OUTPUT_LABEL_IMAGE)
        || outputType.equals(Opt.OUTPUT_BOTH)
        || outputType.equals(Opt.OUTPUT_POLYGONS)))
      return showError(
          String.format(
              "%s must be one of {\"%s\", \"%s\", \"%s\"}.",
              Opt.OUTPUT_TYPE, Opt.OUTPUT_ROI_MANAGER, Opt.OUTPUT_LABEL_IMAGE, Opt.OUTPUT_BOTH));

    if (outputType.equals(Opt.OUTPUT_POLYGONS) && probAxes.contains(Axes.TIME))
      return showError(
          String.format("Timelapse not supported for output type \"%s\"", Opt.OUTPUT_POLYGONS));

    if (!(roiPosition.equals(Opt.ROI_POSITION_STACK)
        || roiPosition.equals(Opt.ROI_POSITION_HYPERSTACK)))
      return showError(
          String.format(
              "%s must be one of {\"%s\", \"%s\"}.",
              Opt.ROI_POSITION, Opt.ROI_POSITION_STACK, Opt.ROI_POSITION_HYPERSTACK));
  ...
  public static void main(final String... args) throws Exception {
    ...
    Dataset prob =
        ij.scifio()
            .datasetIO()
            .open(StarDist2DNMS.class.getClassLoader().getResource("blobs_prob.tif").getFile());
    Dataset dist =
        ij.scifio()
            .datasetIO()
            .open(StarDist2DNMS.class.getClassLoader().getResource("blobs_dist.tif").getFile());
