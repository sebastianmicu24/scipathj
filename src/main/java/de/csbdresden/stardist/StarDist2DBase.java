...
public abstract class StarDist2DBase {
  ...
  @Parameter protected UIService ui;

  @Parameter protected CommandService command;

  @Parameter protected DatasetService dataset;

  @Parameter protected StatusService status;

  @Parameter protected LUTService lut;
  ...
  protected void exportROIs(
      Candidates polygons, int framePosition, long numFrames, String roiPosition) {
  ...
  protected void exportLabelImage(Candidates polygons, int framePosition) {
    if (labelImage == null) labelImage = createLabelImage();
    if (framePosition > 0) labelImage.setT(framePosition);
    ...
    // winners are ordered by score -> draw from last to first to give priority to higher scores in
    // case of overlaps
  ...
  protected abstract void exportPolygons(Candidates polygons);

  protected abstract ImagePlus createLabelImage();
  ...
  protected Dataset labelImageToDataset(String outputType) {
    if (outputType.equals(Opt.OUTPUT_LABEL_IMAGE) || outputType.equals(Opt.OUTPUT_BOTH)) {
      if (labelCount > MAX_LABEL_ID) {
        log.error(
            String.format(
                "Found more than %d segments -> label image does contain some repetitive IDs.\n"
                    + "(\"%s\" output instead does not have this problem).",
                MAX_LABEL_ID, Opt.OUTPUT_ROI_MANAGER));
      }
