...
public class Candidates {
  ...
  public Candidates(
      RandomAccessibleInterval<FloatType> prob, RandomAccessibleInterval<FloatType> dist) {
    this(prob, dist, 0.4);
  }

  public Candidates(
      RandomAccessibleInterval<FloatType> prob,
      RandomAccessibleInterval<FloatType> dist,
      double threshold) {
  ...
  public Candidates(
    ...
    if (verbose)
      log.info(
          String.format("Candidates constructor took %d ms", System.currentTimeMillis() - start));
  }
    ...
    for (int ii = 0; ii < n; ii++) {
      ...
      for (int jj = ii + 1; jj < n; jj++) {
        ...
        if (bbox.does_intersect(bboxes.get(j))) {
          ...
          if (overlap > threshold) suppressed[j] = true;
        }
      }
    }
    if (verbose)
      ...
      IntStream.range(ii + 1, n)
          ...
          .forEach(
              jj -> {
                ...
                if (bbox.does_intersect(bboxes.get(j))) {
                  final double area_inter =
                      poly_intersection_area(polygons.get(i), polygons.get(j));
                  final double overlap =
                      area_inter / Math.min(areas.get(i) + 1e-10, areas.get(j) + 1e-10);
                  if (overlap > threshold) suppressed[j] = true;
                }
              });
    }
  ...
  private double poly_intersection_area(final Path a, final Path b) {
    ...
    c.execute(
        ...
        Clipper.PolyFillType.NON_ZERO);
    double area_inter = 0;
    for (Path p : res) area_inter += p.area();
    return area_inter;
  }
