...
@Plugin(
    ...
    menu = {
      @Menu(
          label = MenuConstants.PLUGINS_LABEL,
...
public class StarDist2D extends StarDist2DBase implements Command {
  ...
  private final String msgTitle =
      "<html><table><tr valign='top'><td><h2>Object Detection with Star-convex Shapes</h2><a"
          + " href='https://imagej.net/StarDist'>https://imagej.net/StarDist</a><br/><br/><small>Please"
          + " cite our paper if StarDist was helpful for your research."
          + " Thanks!</small></td><td>&nbsp;&nbsp;<img src='"
  ...
  @Parameter(
      ...
      label = "<html><b>Neural Network Prediction</b></html>")
  ...
  @Parameter(
      label = Opt.MODEL,
      choices = {
        MODEL_DSB2018_HEAVY_AUGMENTATION,
  ...
  @Parameter(
      label = Opt.PERCENTILE_LOW,
      stepSize = "0.1",
      min = "0",
      max = "100",
      style = NumberWidget.SLIDER_STYLE,
      callback = "percentileBottomChanged")
  private double percentileBottom = (double) Opt.getDefault(Opt.PERCENTILE_LOW);

  @Parameter(
      label = Opt.PERCENTILE_HIGH,
      stepSize = "0.1",
      min = "0",
      max = "100",
      style = NumberWidget.SLIDER_STYLE,
      callback = "percentileTopChanged")
  ...
  @Parameter(
      visibility = ItemVisibility.MESSAGE,
      label = "<html><br/><b>NMS Postprocessing</b></html>")
  ...
  @Parameter(
      label = Opt.OUTPUT_TYPE,
      choices = {Opt.OUTPUT_ROI_MANAGER, Opt.OUTPUT_LABEL_IMAGE, Opt.OUTPUT_BOTH},
  ...
  @Parameter(
      visibility = ItemVisibility.MESSAGE,
      label = "<html><br/><b>Advanced Options</b></html>")
  ...
  @Parameter(
      label = Opt.ROI_POSITION,
      choices = {Opt.ROI_POSITION_AUTO, Opt.ROI_POSITION_STACK, Opt.ROI_POSITION_HYPERSTACK},
      style = ChoiceWidget.RADIO_BUTTON_HORIZONTAL_STYLE)
  ...
  private void checkForCSBDeep() {
    try {
      ...
      JOptionPane.showMessageDialog(
          null,
          "<html><p>StarDist relies on the CSBDeep plugin for neural network"
              + " prediction.<br><br>Please install CSBDeep by enabling its update site.<br>Go to"
              + " <code>Help > Update...</code>, then click on <code>Manage update"
              + " sites</code>.<br>Please see <a"
              + " href='https://github.com/csbdeep/csbdeep_website/wiki/CSBDeep-in-Fiji-%E2%80%93-Installation'>https://tinyurl.com/csbdeep-install</a>"
              + " for more details.</p><img src='"
              + getResource("images/csbdeep_updatesite.png")
              + "' width='498' height='324'>",
  ...
  public void run() {
    ...
    if (roiPosition.equals(Opt.ROI_POSITION_AUTO))
      roiPositionActive =
          input.numDimensions() > 3 && !input.isRGBMerged()
              ? Opt.ROI_POSITION_HYPERSTACK
              : Opt.ROI_POSITION_STACK;
    else roiPositionActive = roiPosition;
    ...
    try {
      ...
      switch (modelChoice) {
        ...
        default:
          ...
          if (pretrainedModel.canGetFile()) {
            ...
            if (pretrainedModel.isTempFile()) tmpModelFile = file;
          } else {
      ...
      if (true && isTimelapse) {
        ...
        final int inputTimeDim =
            IntStream.range(0, inputAxes.size())
                .filter(d -> input.axis(d).type() == Axes.TIME)
                .findFirst()
                .getAsInt();
        for (int t = 0; t < numFrames; t++) {
          final Dataset inputFrameDS =
              Utils.raiToDataset(
                  dataset,
                  "Input Frame",
          ...
          final Future<CommandModule> futureCNN =
              command.run(de.csbdresden.csbdeep.commands.GenericNetwork.class, false, paramsCNN);
          ...
          if (showProbAndDist) {
            ...
            if (t == 0)
              log.error(
                  String.format(
                      "\"%s\" not implemented/supported for timelapse data.", Opt.SHOW_PROB_DIST));
          }

          final Future<CommandModule> futureNMS =
              command.run(StarDist2DNMS.class, false, paramsNMS);
      ...
      try {
        if (tmpModelFile != null && tmpModelFile.exists()) tmpModelFile.delete();
      } catch (Exception e) {
        e.printStackTrace();
      }
    }
  ...
  private Pair<Dataset, Dataset> splitPrediction(final Dataset prediction) {
    final RandomAccessibleInterval<FloatType> predictionRAI =
        (RandomAccessibleInterval<FloatType>) prediction.getImgPlus();
    final LinkedHashSet<AxisType> predAxes = Utils.orderedAxesSet(prediction);

    final int predChannelDim =
        IntStream.range(0, predAxes.size())
            .filter(d -> prediction.axis(d).type() == Axes.CHANNEL)
            .findFirst()
            .getAsInt();
    final long[] predStart =
        predAxes.stream()
            .mapToLong(
                axis -> {
    ...
    final long[] predSize =
        predAxes.stream()
            .mapToLong(
                axis -> {
                  return axis == Axes.CHANNEL
                      ? prediction.dimension(axis) - 1
                      : prediction.dimension(axis);
                })
            .toArray();

    final RandomAccessibleInterval<FloatType> probRAI =
        Views.hyperSlice(predictionRAI, predChannelDim, 0);
    final RandomAccessibleInterval<FloatType> distRAI =
        Views.offsetInterval(predictionRAI, predStart, predSize);

    final Dataset probDS =
        Utils.raiToDataset(
            dataset,
            Opt.PROB_IMAGE,
            probRAI,
  ...
  private boolean checkInputs() {
    ...
    if (!((input.numDimensions() == 2 && axes.containsAll(Arrays.asList(Axes.X, Axes.Y)))
        || (input.numDimensions() == 3
            && axes.containsAll(Arrays.asList(Axes.X, Axes.Y, Axes.TIME)))
        || (input.numDimensions() == 3
            && axes.containsAll(Arrays.asList(Axes.X, Axes.Y, Axes.CHANNEL)))
        || (input.numDimensions() == 4
            && axes.containsAll(Arrays.asList(Axes.X, Axes.Y, Axes.CHANNEL, Axes.TIME)))))
      return showError("Input must be a 2D image or timelapse (with or without channels).");

    if (!(modelChoice.equals(Opt.MODEL_FILE)
        || modelChoice.equals(Opt.MODEL_URL)
        || MODELS.containsKey(modelChoice)))
      return showError(String.format("Unsupported Model \"%s\".", modelChoice));

    if (!(roiPosition.equals(Opt.ROI_POSITION_AUTO)
        || roiPosition.equals(Opt.ROI_POSITION_STACK)
        || roiPosition.equals(Opt.ROI_POSITION_HYPERSTACK)))
      return showError(
          String.format(
              "%s must be one of {\"%s\", \"%s\", \"%s\"}.",
              Opt.ROI_POSITION,
              Opt.ROI_POSITION_AUTO,
              Opt.ROI_POSITION_STACK,
              Opt.ROI_POSITION_HYPERSTACK));
  ...
  public static void main(final String... args) throws Exception {
    ...
    Dataset input =
        ij.scifio()
            .datasetIO()
            .open(StarDist2D.class.getClassLoader().getResource("yeast_crop.tif").getFile());
    //        Dataset input =
    // ij.scifio().datasetIO().open(StarDist2D.class.getClassLoader().getResource("yeast_timelapse.tif").getFile());
    //        Dataset input =
    // ij.scifio().datasetIO().open(StarDist2D.class.getClassLoader().getResource("patho_hyperstack.tif").getFile());

    //        Dataset input =
    // ij.scifio().datasetIO().open(StarDist2D.class.getClassLoader().getResource("data/test_image_fluo.tif").getFile());
    //        Dataset input =
    // ij.scifio().datasetIO().open(StarDist2D.class.getClassLoader().getResource("data/test_image_fluo_timelapse.tif").getFile());
    //        Dataset input =
    // ij.scifio().datasetIO().open(StarDist2D.class.getClassLoader().getResource("data/test_image_histo.tif").getFile());
